...
def get_task(task_id: int = Path(..., ge=1)) -> TaskRead:
...
def update_task(task_id: int = Path(..., ge=1), payload: TaskUpdate | None = None) -> TaskRead:
    """Update fields on an existing task.

    Accepts a partial payload; unspecified fields remain unchanged.
    """
    try:
        task = TaskService.update_task(task_id, payload or TaskUpdate())
    except TaskNotFoundError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc))
    return _to_schema(task)
...
def delete_task(task_id: int = Path(..., ge=1)) -> Response:
